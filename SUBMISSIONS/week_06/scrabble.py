...
if len(sys.argv) == 1:
    raise Exception("Characters should be entered.\
        You did not enter characters argument.")
if len(sys.argv) > 2:
    raise Exception("Only one argument should be entered (characters).\
                     You have entered more than one argument.")

# Allow anywhere from 2-7 character tiles to be inputted
if len(sys.argv[1]) < 2 or len(sys.argv[1]) > 7:
    raise Exception("Enter 2-7 characters. You have entered less than 2 or\
...
if chars.count('*') + chars.count('?') > 2:
    raise Exception("Make sure to enter up to two wildcards ('*' or '?').\
                     You have entered more than two.")

# Entered a character not defined in the dictionary
for char in chars:
    if char.lower() not in scores.keys():
	    raise Exception("Invalid character: \'" + char + "\'. Enter only\
                         English alphabet characters or wildcards ('*' or '?')")
...
for word in data:
    ...
    word = word.lower()
    # Check if each character in the word exists in the tiles\
    #  available (or if a wildcard can be used)
    for i in range(len(word)):
        if word[i] in rack:
            ...
            break
    # If all the characters in the word can be formed from the tiles,\
    #  add word and score to results
    else:
        word_score = word_score.lower()
        results.append((score_word(word_score, scores), word.lower()))

# Sort words by score (with insertion sort algorithm)
